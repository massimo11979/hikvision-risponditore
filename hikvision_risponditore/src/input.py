...
class InputReader():
    ...
    def execute_command(self, command: str):
        ...
        match arguments[0]:
            ...
            case "callAudio":
                if len(arguments) < 3:
                    logger.error("Usage: callAudio <doorbell_name> <audio_path_or_url>")
                    return
                audio_path = " ".join(arguments[2:])
                logger.info("Starting call audio for {} from {}", doorbell._config.name, audio_path)
                doorbell.start_voice_talk(audio_file_path=audio_path)
            case "broadcastOn":
